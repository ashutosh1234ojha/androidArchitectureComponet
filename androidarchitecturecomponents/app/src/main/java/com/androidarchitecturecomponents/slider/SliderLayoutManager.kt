...
//            scaleDownView()
    ...
    override fun onScrollStateChanged(state: Int) {
        ...
        if (state.equals(RecyclerView.SCROLL_STATE_IDLE)) {
            ...
            Log.e("RV", "rv center $recyclerViewCenterX")
            var position = -1
            for (i in 0 until recyclerView.childCount) {
                ...
                Log.e("RV", "childCenterX $childCenterX")
                if (newDistance < minDistance) {
                    minDistance = newDistance
                    position = recyclerView.getChildLayoutPosition(child)

                    childCenter = childCenterX

                    Log.e("RV", "minDistance $minDistance")
                    Log.e("RV", "position $position")

                }


            }
            recyclerView.scrollX=childCenter
